...
The curve is generated by an instance of the engine's own `ShapeTable`, not by a
second copy of the formula, so what is drawn is the curve the audio takes —
including the [1/32, 32] clamp on `p` and the table's quantisation. Note that
this is a **power curve, not a sigmoid**: at the extremes it has one flat run and
one steep one, not a flat–steep–flat S.
